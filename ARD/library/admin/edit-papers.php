...
$fileExistsFlag = 0; 
	$fileName = $_FILES['linkk']['name'];

	if($fileExistsFlag == 0) { 
		$target = "papers/";		
		$fileTarget = $target.$refname.$fileName;	
		$tempFileName = $_FILES["linkk"]["tmp_name"];
...
<!-- MENU SECTION END-->
    ...
    <div class="content-wrapper">
        ...
        <div class="row pad-botm">
            <div class="col-md-12">
                <h4 class="header-line">Edit Past Papers</h4>
...
<?php 
$refid=intval($_GET['refid']);
$sql = "SELECT * from digital where id=:refid and  type='papers' ";
$query = $dbh -> prepare($sql);
$query->bindParam(':refid',$refid,PDO::PARAM_STR);
$query->execute();
...
<input class="form-control" type="file" name="linkk"  required />
</div>
